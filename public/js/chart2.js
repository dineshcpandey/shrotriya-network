...
/**
 * Force all nodes to display (override family chart library behavior)
 */
function forceDisplayAllNodes() {
    // Wait for chart to render
    setTimeout(() => {
        console.log('Forcing all nodes to display...');

        // Method 1: Show all hidden card containers
        const hiddenCards = document.querySelectorAll('#FamilyChart .card_cont[style*="display: none"], #FamilyChart .card_cont[style*="opacity: 0"]');
        ...
        // Method 2: Show all SVG groups
        const svgGroups = document.querySelectorAll('#FamilyChart svg g');
        ...
        // Method 3: Override any transform hiding
        const allCards = document.querySelectorAll('#FamilyChart .card_cont');
        allCards.forEach(card => {
            // Reset any hiding transforms
            const transform = card.style.transform;
            if (transform && transform.includes('scale(0)')) {
                card.style.transform = transform.replace(/scale\([^)]*\)/g, 'scale(1)');
            }
        });

        // Method 4: Force chart to show all data
        if (f3Chart && f3Chart.data) {
            const allData = f3Chart.data();
            console.log('Chart has data for', allData.length, 'nodes');

            // Force update with all data
            f3Chart.updateData(allData);
            f3Chart.updateTree();
        }

        console.log('All nodes forced to display');
    }, 1500);
}

// Call this function after chart initialization
// In your initializeChart function, add:
f3Chart.updateTree({ initial: true });
forceDisplayAllNodes();
