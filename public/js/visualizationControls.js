...
/**
 * Handle edit form state changes to adjust controls positioning
 * @param {boolean} isOpen - Whether the edit form is open
 */
export function handleEditFormStateChange(isOpen) {
    const controlsContainer = document.getElementById('visualization-controls');
    if (!controlsContainer) return;

    console.log('Edit form state changed:', isOpen);

    if (isOpen) {
        controlsContainer.classList.add('edit-form-open');
        console.log('Added edit-form-open class to controls container');
    } else {
        controlsContainer.classList.remove('edit-form-open');
        console.log('Removed edit-form-open class from controls container');
    }
}

/**
 * Setup observers to detect edit form visibility changes
 */
function setupEditFormObserver() {
    const editForm = document.getElementById('edit-form');
    if (!editForm) {
        console.log('Edit form not found, observer not set up');
        return;
    }

    console.log('Setting up edit form observer');

    // Create a MutationObserver to watch for class changes
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                const isVisible = editForm.classList.contains('visible');
                console.log('Edit form class changed, visible:', isVisible);
                handleEditFormStateChange(isVisible);
            }
        });
    });
    ...
    const isVisible = editForm.classList.contains('visible');
    console.log('Initial edit form visibility:', isVisible);
    handleEditFormStateChange(isVisible);
}